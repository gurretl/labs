...
def fetch_rss(previous_month):
    ...
    matching_dirs = [d for d in os.listdir(blog_dir) if os.path.isdir(os.path.join(blog_dir, d)) and f"azure_updates_{previous_month.strftime('%m_%Y')}" in d]

    # If a corresponding directory exists, use its directory and retrieve the prefix number
    if matching_dirs:
        ...
        # If no corresponding directory exists, create a new directory with the appropriate prefix
        existing_prefixes = [int(d.split("_")[0]) for d in os.listdir(blog_dir) if d.startswith(tuple(str(x).zfill(3) for x in range(1, 20)))]
